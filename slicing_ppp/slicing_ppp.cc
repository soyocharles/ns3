...
runsim (data_input input)
{
    ...
    for (uint32_t u = 0; u < ueNodes_1.GetN (); ++u)
    {
        ...
        c->SetAttribute("PacketSize",UintegerValue (100));
    ...
    for (uint32_t u = 0; u < ueNodes_2.GetN (); ++u)
    {
        Ptr<UdpClientLatency> c = CreateObject<UdpClientLatency> ();
        c->SetAttribute("MaxPackets",UintegerValue (np));
        c->SetAttribute("Interval",TimeValue(MicroSeconds (80000)));
    ...
    for (uint32_t u = 0; u < ueNodes_1.GetN (); ++u){
        ...
        int ueimsi  = ueNodes_1.Get(u)->GetDevice(1)->GetObject<LteUeNetDevice>()->GetImsi();
        filename <<"./share/" <<"effcqitest"<< "_" << (int)(input.mean_ue_1) << "_" << (int)(input.mean_ue_2) << "_" << seed << "_" << (int)0 << "_" << 1 << "_" << ueimsi <<".csv";
    ...
    for (uint32_t u = 0; u < ueNodes_2.GetN (); ++u){
        std::ostringstream filename;
        int ueimsi  = ueNodes_2.Get(u)->GetDevice(1)->GetObject<LteUeNetDevice>()->GetImsi();
        filename <<"./share/" <<"effcqitest"<< "_" << (int)(input.mean_ue_1) << "_" << (int)(input.mean_ue_2) << "_" << seed << "_" << (int)0 << "_" << 2 << "_" << ueimsi <<".csv";
        outfile.open(filename.str(), std::ofstream::out | std::ofstream::trunc);
        std::vector< std::pair<uint32_t, std::pair<double,double>> >::iterator it;
        Ptr<UdpServerLatency> s = servers_2.at(u);
        for(it = s->m_data.begin();it != s->m_data.end(); it++){
...
main(int argc, char* argv[]) {
    int seed_from = 1;
    CommandLine cmd;
    cmd.AddValue("seed_from", "The seed number from", seed_from);
    cmd.Parse (argc, argv);
    std::cout<< "simulation seed from" << seed_from << std::endl;
    int mean_ue_1 = 100;
    int mean_ue_2 = 150;

    for(int seed = seed_from; seed < seed_from + 1 ; seed++){
        // determine the number of ues and enbs
        std::default_random_engine generator (seed);
        std::poisson_distribution<int> distribution_ue_1 (mean_ue_1);
        std::poisson_distribution<int> distribution_ue_2 (mean_ue_2);
        std::poisson_distribution<int> distribution_enb (4);


        data_input input;
        input.seed = seed;
        input.num_enb =  4;
        if (input.num_enb == 0) input.num_enb =1;
        input.num_ue_1 = distribution_ue_1(generator);
        input.num_ue_2 = distribution_ue_2(generator);
        input.mean_ue_1 = mean_ue_1;
        input.mean_ue_2 = mean_ue_2;
        input.sim_time = 1.6;
        input.area = 500;
...
//        input.num_block = 10;
//        std::cout << "simulation slpf "<< input.num_block << std::endl;
//        input.ss = 4;
//        input.filename = time(0);            
//        runsim(input);
        ...
        RngSeedManager::SetRun(1);  
        input.num_block = 0;
        std::cout << "simulation pf "<< input.num_block << std::endl;
        input.ss = 1;
        input.filename = time(0);            
        runsim(input);
